...
const milestoneColors = [
  ...
  { min: 7, max: 13, message: ' 빨강 무지개를 완성했어요.', color: 'bg-red-500' },
  { min: 14, max: 20, message: ' 주황 무지개를 완성했어요.', color: 'bg-orange-500' },
  { min: 21, max: 27, message: ' 노랑 무지개를 완성했어요.', color: 'bg-yellow-400' },
  { min: 28, max: 34, message: ' 초록 무지개를 완성했어요.', color: 'bg-green-500' },
  { min: 35, max: 41, message: ' 파랑 무지개를 완성했어요.', color: 'bg-blue-500' },
  { min: 42, max: 48, message: ' 남색 무지개를 완성했어요.', color: 'bg-indigo-600' },
  { min: 49, max: 49, message: ' 모든 무지개를 달성했어요!', color: 'bg-purple-600' },
];
...
export const ProgressCard: React.FC<ProgressCardProps> = ({
  ...
  return (
    <div
      className={`relative flex flex-col justify-center items-center w-full max-w-md p-4 bg-[#f3f6fb] shadow-[0px_4px_4px_#00000040] rounded-lg ${className}`}
    >
      ...
      <div className="flex flex-col items-center justify-center w-full mt-4 bg-white rounded-lg shadow-[0px_4px_8px_#dbe5ec99,0px_0px_1px_1px_#dbe5ec99] p-4">
        <div className="w-full">
          <ProgressBar className="w-full h-1 bg-gray-200 rounded-lg" fill={fill} />
        </div>
